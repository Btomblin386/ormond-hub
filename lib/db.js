...
export function tasksForClient(clientId) {
  return withDb((sql) => sql`
    select id, kind, title, instructions, status, result, error, acknowledged, created_by, assigned_to,
...
export function createTask(d) {
  return withDb((sql) => sql`
    ...
    values (${d.clientId}, ${d.kind}, ${d.title}, ${d.instructions || null}, ${d.dueAt || null},
...
// Fire-and-forget trigger of the account-task runner via pg_net, so the hub
// request returns immediately and the edge function does the slow work.
export function triggerTaskRunner(taskId, anonKey) {
  return withDb((sql) => sql`
    select net.http_post(
      url := 'https://jxlrnuyfracyygiksqdj.supabase.co/functions/v1/account-task',
      headers := jsonb_build_object('Content-Type','application/json','Authorization', ${"Bearer " + anonKey}),
      body := ${JSON.stringify({ task_id: taskId })}::jsonb
    ) as id`);
}
...
export function mergeBrandSettings(clientId, patch) {
  return withDb((sql) => sql`
    update clients set brand_settings = coalesce(brand_settings, '{}'::jsonb) || ${JSON.stringify(patch || {})}::jsonb
    where id = ${clientId}`);
}
...
export function agencyNotifications() {
  return withDb(async (sql) => {
    ...
    const stuck = await sql`
      ...
      from content_items ci join clients c on c.id = ci.client_id
      where ci.status = 'publishing' and ci.updated_at < now() - interval '20 minutes'
