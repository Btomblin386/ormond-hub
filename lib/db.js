...
export function accountById(id) {
  return withDb(async (sql) => {
    const [row] = await sql`
      select a.id, a.client_id, c.name as client, a.platform, a.external_account_id, a.currency,
...
export function contentForClient(clientId) {
  return withDb((sql) => sql`
    select id, channels, caption, link, media_urls, status,
           scheduled_at::text as scheduled_at, published_at::text as published_at,
           fb_post_id, ig_post_id, error, created_at::text as created_at
    from content_items
...
export function contentCalendar() {
  return withDb((sql) => sql`
    select ci.id, c.name as client, ci.client_id, ci.channels, ci.caption,
           ci.media_urls, ci.status, ci.scheduled_at::text as scheduled_at,
...
export function createContent(d) {
  return withDb((sql) => sql`
    insert into content_items (client_id, channels, caption, link, media_urls, status, scheduled_at, created_by)
    values (${d.clientId}, ${sql.array(d.channels)}, ${d.caption || ""}, ${d.link || null},
            ${sql.array(d.mediaUrls || [])}, ${d.status || "draft"},
...
export function updateContent(id, d) {
  return withDb((sql) => sql`
    update content_items set
      channels = ${sql.array(d.channels)},
      caption = ${d.caption || ""},
      link = ${d.link || null},
      media_urls = ${sql.array(d.mediaUrls || [])},
...
export function mentionsForClient(clientId) {
  return withDb((sql) => sql`
    select id, title, snippet, url, author, sentiment, tags,
...
export function agencyNotifications() {
  return withDb(async (sql) => {
    ...
    const failed = await sql`
      select ci.id, c.name as client, ci.error, ci.updated_at::text as created_at
      from content_items ci join clients c on c.id = ci.client_id
    ...
    return { todos, failed, ruleEvents, insights };
  });
}
