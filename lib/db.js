...
export function tasksForClient(clientId) {
  return withDb((sql) => sql`
    select id, kind, title, instructions, status, result, error, acknowledged,
...
export function createTask(d) {
  return withDb((sql) => sql`
    insert into account_tasks (client_id, kind, title, instructions, due_at, status)
    values (${d.clientId}, ${d.kind}, ${d.title}, ${d.instructions || null}, ${d.dueAt || null},
            ${d.kind === "reminder" ? "pending" : "queued"})
    returning id`);
}
