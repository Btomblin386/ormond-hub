...
export function listHtmlToText(html) {
  ...
  // Paragraphs keep their blank-line separation; list items stay tight,
  // with one blank line after the whole list.
  const walkBlock = (el) => {
    for (const n of el.childNodes) {
      if (n.nodeType === 3) { const t = n.textContent.trim(); if (t) lines.push(t, ""); continue; }
      if (n.nodeType !== 1) continue;
      if (n.nodeName === "UL" || n.nodeName === "OL") { walkList(n, 0); lines.push(""); }
