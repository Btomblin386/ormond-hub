...
// Upload a video (or any large file) straight to storage via a one-time signed
// URL — the file never gets base64'd through JSON, so multi-MB clips work.
// Returns { url } (public URL) on success.
export function uploadVideoSigned({ file, onProgress }) {
  return new Promise((resolve, reject) => {
    fetch("/api/content-media", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ sign: true, filename: file.name, contentType: file.type }),
    })
      .then((r) => r.json())
      .then((s) => {
        if (!s || !s.signedUrl) return reject(new Error(s?.error || "could not start upload"));
        const xhr = new XMLHttpRequest();
        xhr.open("PUT", s.signedUrl);
        xhr.setRequestHeader("Content-Type", file.type || "application/octet-stream");
        xhr.setRequestHeader("x-upsert", "true");
        xhr.upload.onprogress = (e) => { if (e.lengthComputable && onProgress) onProgress(Math.round((e.loaded / e.total) * 100)); };
        xhr.onload = () => {
          if (xhr.status >= 200 && xhr.status < 300) resolve({ url: s.publicUrl });
...
// Canvas data URL -> Blob (so editor output can use the signed upload path
// instead of the size-capped base64 JSON route).
