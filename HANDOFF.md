...
- **Composer Post Now + formatting:** ⚡ Post now button (save approved with immediate schedule + fire content-publish now via new `publishNow` flag → `kickPublisher()` in /api/content; cron still backs it up). FormatBar above each caption does **Bold/Italic/Clear** via Unicode math-alphabet chars (IG/FB show no markdown) — transforms the selected text, reversible via TO_PLAIN map.
- **Calendar quick-view time edit:** the post preview modal now has a datetime-local + "Update time" (op reschedule) for not-yet-published posts. Added `toLocalInput()` helper.
- **Dropbox flaky Hidden toggle fixed:** DropboxFolderControl was optimistic-only and read stale `e.hidden`; now serializes writes (`if (busy) return`), trusts the server's returned `hidden` list, and rebuilds the prefix set from each admin list response so the UI can't drift.
- **Password reset / reactivate (Team & access):** new users route ops `reset_password` (hashPassword → setUserPassword, reactivates) and `reactivate`; TeamManager has per-user "Reset password" (with Generate) + Reactivate for deactivated logins. db: `reactivateUser`, `setUserPassword`.
- **Credentials panel:** dropped the **unused** `BRAND_LISTENER_API_KEY` (confirmed: all FB/IG listening runs on META_ACCESS_TOKEN; the key was only referenced by the never-wired `api` source kind). New components/CredentialsPanel.js — each row expands to exact "how to update" steps (Supabase secrets vs Vercel env). Added DASHBOARD_PASSWORD to the list, flagged as the login+session key.
- **Payment-failure / spend-drop alert + dismissable notices:** new `notif_dismissals(dismiss_key pk)` table + `dismissNotification` + /api/notifications (agency-gated). agencyNotifications now returns `spendDrops` (accounts averaging ≥$5/day whose last 2 data-days collapsed to <15% of expected — caught Slavens Racing at $0 vs $44/day avg, a real payment lapse) and every informational card (spendDrops/failed/stuck/insights/overdue) carries a stable `_key` with a × dismiss in NotificationsFeed. Note: data lags ~1 day so weekend outages surface Monday.
